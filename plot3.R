...
plot3 <- function(){
  ...
  with(dat, {
    ...
    a=legend("topright", legend = c("Sub_metering_1","Sub_metering_2", "Sub_metering_3"), col = c("black", "red", "blue"), lty=1, y.intersp=0.25,plot=F)
    a=a$rect
    rect(xleft = a$left+0.2*a$w, ytop = a$top + a$h, xright = a$left+a$w, ybottom = a$top-0.5*a$h)
  })
